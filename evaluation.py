...
def plot_rmse(first_quartile, median, third_quartile, x_axis=None, algorithm=''):
	plt.figure(1)
	if x_axis is None:
		x_axis = np.arange(median.shape[0])
	color = next(COLORS)
	plot1, = plt.plot(x_axis, median, 'k-', color=color, label=algorithm)
	plot2 = plt.fill_between(x_axis, first_quartile, third_quartile, color=color, label=algorithm, alpha=0.3)
	axes = plt.gca()
	return plot1, plot2, axes

def plot_rmse_vs_sigma(*args, **kwargs):
...
def plot_rmse_vs_tags(*args, **kwargs):
	plot1, plot2, axes = plot_rmse(*args, **kwargs)
